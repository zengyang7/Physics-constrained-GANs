...
def generator(z, isTrain=True, reuse=False):
    with tf.variable_scope('generator', reuse=reuse):
        ...
        # residual layers
        convR1 = tf.layers.conv2d(lrelu3, 64, [1, 1], strides=(1, 1), padding='same', 
                                             kernel_initializer=w_init, bias_initializer=b_init)
        lrelu_R1 = lrelu(tf.layers.batch_normalization(convR1, training=isTrain), 0.2)
        
        convR2 = tf.layers.conv2d(lrelu_R1, 64, [5, 5], strides=(1, 1), padding='same', 
                                             kernel_initializer=w_init, bias_initializer=b_init)
        lrelu_R2 = lrelu(tf.layers.batch_normalization(convR2, training=isTrain), 0.2)
        
        convR3 = tf.layers.conv2d(lrelu_R2, 64, [1, 1], strides=(1, 1), padding='same', 
                                             kernel_initializer=w_init, bias_initializer=b_init)
        lrelu_R3 = lrelu(tf.layers.batch_normalization(convR3, training=isTrain), 0.2)
        
        add = tf.add(lrelu3, lrelu_R3)
        
        # output layer
        deconv3 = tf.layers.conv2d_transpose(add, 3, [5, 5], strides=(1, 1), padding='same', 
                                             kernel_initializer=w_init, bias_initializer=b_init)
...
def discriminator(x, isTrain=True, reuse=False):
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        # 2nd hidden layer
        conv2 = tf.layers.conv2d(lrelu2, 64, [5, 5], strides=(1, 1), padding='same', kernel_initializer=w_init, bias_initializer=b_init)
        lrelu2 = lrelu(tf.layers.batch_normalization(conv2, training=isTrain), 0.2)
...
name_data = root + 'PF-oWGANGP-Res-cons'+str(cons_value)+'-lam'+str(lam_cons)+'-lr'+str(lr_setting)+'-ep'+str(train_epoch)
