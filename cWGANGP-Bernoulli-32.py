...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 28 21:55:17 2018
...
def generator(z, y_label, isTrain=True, reuse=False):
    with tf.variable_scope('generator', reuse=reuse):
        ...
        deconv1 = tf.layers.conv2d_transpose(cat1, 256, [4, 4], strides=(1, 1), padding='valid', 
...
def discriminator(x, y_fill, isTrain=True, reuse=False):
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        # output layer
        conv4 = tf.layers.conv2d(lrelu3, 1, [4, 4], strides=(1, 1), padding='valid', kernel_initializer=w_init)
...
def constraints(x, dx, dy, filtertf):
    ...
    #due to the numerical errors, the divergence in the center is very large,
    #it is filted by the filter
    ...
    delta_soft_ = divergence_mean - kesi
    delta_soft_ = tf.nn.relu(delta_soft_)
    return delta_soft_, divergence_mean
...
for epoch in range(train_epoch+1):
    ...
    for iter in range(train_set.shape[0] // batch_size):
        ...
        y_label_ = train_label[iter*batch_size:(iter+1)*batch_size].reshape([batch_size, 1, 1, n_label])
        y_fill_ = y_label_ * np.ones([batch_size, n_mesh, n_mesh, n_label])
        
        loss_d_, _ = sess.run([D_loss, D_optim], {x:x_, z:z_, y_label:y_label_, y_fill:y_fill_, isTrain: True})
        
        # update generator
        z_ = np.random.normal(0, 1, (batch_size, 1, 1, 100))
        loss_g_, _ = sess.run([G_loss, G_optim], {z:z_, x:x_, dx:d_x_, dy:d_y_, y_label:y_label_, y_fill:y_fill_, filtertf:filter_batch, isTrain: True})

        errD = D_loss.eval({z:z_, x:x_, y_label:y_label_, y_fill:y_fill_, filtertf:filter_batch, isTrain: False})
        errG = G_loss_only.eval({z: z_, dx:d_x_, dy:d_y_, y_label:y_label_, y_fill:y_fill_, filtertf:filter_batch, isTrain: False})
        errdelta_real = divergence_mean.eval({z:z_, dx:d_x_, y_label:y_label_, y_fill:y_fill_, dy:d_y_,filtertf:filter_batch, isTrain: False})
        errdelta_lose = delta_lose.eval({z: z_, dx:d_x_, dy:d_y_, y_label:y_label_, y_fill:y_fill_, filtertf:filter_batch, isTrain: False})
    ...
    y_label_pred = train_label[0:16].reshape([16, 1, 1, n_label])
    ...
    if epoch % 20 == 0:
        ...
        y_label_pred = train_label[0:2000].reshape([2000, 1, 1, n_label])
        prediction = G_z.eval({z:z_pred, y_label:y_label_pred, isTrain: False})
        prediction[:,:,:,0:2] = prediction[:,:,:,0:2]*(1.1*(nor_max_v-nor_min_v)/2)+(nor_max_v+nor_min_v)/2
        prediction[:,:,:,2] = prediction[:,:,:,2]*(1.1*(nor_max_p-nor_min_p)/2)+(nor_max_p+nor_min_p)/2
