...
def generator(z, isTrain=True, reuse=False):
    with tf.variable_scope('generator', reuse=reuse):
        ...
        # output layer
        deconv4 = tf.layers.conv2d_transpose(lrelu3, 3, [5, 5], strides=(1, 1), padding='same', 
                                             kernel_initializer=w_init, bias_initializer=b_init)
...
def discriminator(x, isTrain=True, reuse=False):
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        # 3nd hidden layer
        conv3 = tf.layers.conv2d(lrelu2, 64, [5, 5], strides=(1, 1), padding='same', kernel_initializer=w_init, bias_initializer=b_init)
        lrelu3 = lrelu(tf.layers.batch_normalization(conv3, training=isTrain), 0.2)

        # output layer
        conv4 = tf.layers.conv2d(lrelu3, 1, [7, 7], strides=(1, 1), padding='valid', kernel_initializer=w_init)
