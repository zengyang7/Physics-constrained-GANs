...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 18 15:32:44 2018
...
def plot_samples(X, Y, U,lvls, name=None):
    '''
    plot the samples
    '''
    figm, axsm = plt.subplots(2, 2, figsize=(15, 10))
    figp, axsp = plt.subplots(2, 2, figsize=(15, 10))
    figm.subplots_adjust(hspace = .05, wspace=.01)
    figp.subplots_adjust(hspace = .05, wspace=.01)
    axsm = axsm.ravel()
    axsp = axsp.ravel()
    data_num = len(U)
    num_plot = np.min([data_num, 4])
    for i in range(num_plot):
        sample = U[i]
        u = sample[:,:,0]
        v = sample[:,:,1]
        M = np.sqrt(u**2+v**2)
        axsm[i].contourf(X, Y, M, levels=lvls,cmap=plt.cm.hot)
        axsm[i].streamplot(X, Y, u, v,density=0.6,linewidth=0.8,color='k')
        axsm[i].set_xticklabels([])
        axsm[i].set_yticklabels([])
        
        axsp[i].contourf(X, Y, sample[:,:,2],cmap=plt.cm.hot)
        axsp[i].set_xticklabels([])
        axsp[i].set_yticklabels([])
    plt.show()
    if name != None:
        namem = name+'m.png'
        namep = name+'p.png'
...
Magni = np.sqrt(U[:,:,:,0]**2+U[:,:,:,1]**2)
Mag_max = np.max(Magni)
Mag_min = np.min(Magni)
lvls = np.linspace(Mag_min, Mag_max, 30)
...
def generator(z, isTrain=True, reuse=False):
    ...
    with tf.variable_scope('generator', reuse=reuse):
        ...
        lrelu1 = lrelu(deconv1, 0.2)
        lrelu1 = tf.layers.dropout(lrelu1, keep_prob)
        lrelu1 = tf.layers.batch_normalization(lrelu1, training=isTrain)
        

        # 2nd hidden layer
        deconv2 = tf.layers.conv2d_transpose(lrelu1, 128, [5, 5], strides=(2, 2), padding='same', 
                                             kernel_initializer=w_init, bias_initializer=b_init)
        lrelu2 = lrelu(deconv2, 0.2)
        lrelu2 = tf.layers.dropout(lrelu2, keep_prob)
        lrelu2 = tf.layers.batch_normalization(lrelu2, training=isTrain)

        # output layer
        deconv3 = tf.layers.conv2d_transpose(lrelu2, 3, [5, 5], strides=(2, 2), padding='same', 
                                             kernel_initializer=w_init, bias_initializer=b_init)
...
# variables : input
x = tf.placeholder(tf.float32, shape=(None, n_mesh, n_mesh, 3))
...
name_data = root + 'PF-oWGANGP-Ber-Dropout-cons'+str(cons_value)+'-lam'+str(lam_cons)+'-lr'+str(lr_setting)+'-ep'+str(train_epoch)
